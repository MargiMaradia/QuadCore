      ...
      >
        ...
        <div className="p-6 border-b border-silk-clay bg-gray-50/50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full border border-silk-clay bg-silk-charcoal text-silk-gold flex items-center justify-center font-bold text-sm">
              {(user?.name || user?.fullName || user?.email || 'U').charAt(0).toUpperCase()}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-bold text-silk-charcoal truncate">{user?.name || user?.fullName || user?.email || 'User'}</p>
              <p className="text-xs text-silk-mauve capitalize truncate">{user?.role?.replace('_', ' ') || 'User'}</p>
            </div>
          </div>
        </div>
